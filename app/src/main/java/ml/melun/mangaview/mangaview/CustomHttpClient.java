...
public class CustomHttpClient {
    ...
    private synchronized String getCookieHeaderForNtkPath(String path) {
        ...
        for(String key : cookies.keySet()) {
            String value = cookies.get(key);
            if(("ad_ack".equals(key) || "ad_ack_c".equals(key))
                    && !ntkAdAckCookieMatchesPath(value, path))
                continue;
            if(builder.length() > 0)
                builder.append("; ");
    ...
    public List<String> fetchNtkViewerImageUrls(String segment, String workId, String episodeId,
        ...
        FutureTask<List<String>> task = new FutureTask<>(() ->
                fetchNtkViewerImageUrlsUncached(kind, endpoint, baseUrl, path, segment,
                        workId, episodeId, imagesToken, cacheKey));
        ...
        try {
            ...
            return result == null ? urls : result;
        } catch(InterruptedException e) {
            Thread.currentThread().interrupt();
            return urls;
        } catch(Exception e) {
        ...
        for(int attempt = 0; attempt < 2; attempt++) {
            try {
                NtkQuicFetcher.Result result = fetchNtkQuic(baseUrl, baseUrl + endpoint,
                        getCookieHeaderForNtkPath(path), headers, "POST", body, 12000L);
    ...
    private boolean performNtkNativeAckBypassLocked(String baseUrl, String path, String cacheKey) {
        ...
        try {
            ...
            Log.d(TAG, "ntk_native_ack_token_before_canary len=" + token.length() + ",path=" + challengePath);
            ...
            boolean canaryPending = false;
            try {
                NtkAckCanaryResult canaryResult = canaryFuture.get(250L, TimeUnit.MILLISECONDS);
                ...
                canaryPending = true;
                Log.d(TAG, "ntk_native_ack_canary_pending path=" + challengePath);
            }
            ...
            for(int attempt = 0; attempt < NTK_ACK_REQUEST_ATTEMPTS; attempt++) {
                if(attempt > 0) Thread.sleep(NTK_ACK_RETRY_DELAY_MS * attempt);
                ack = NtkQuicFetcher.fetchWithEngine(engine, executor, baseUrl + "/api/ad/ack", agent,
                        getCookieHeaderForNtkPath(challengePath), h2, "POST", ackBytes, NTK_ACK_CONFIRM_TIMEOUT_MS);
                ...
                if(canaryPending) {
                    canaryPending = false;
                    try {
                        NtkAckCanaryResult canaryResult = canaryFuture.get(3500L, TimeUnit.MILLISECONDS);
                        if(canaryResult.canary != null)
                            applySetCookieHeaders(canaryResult.canary.headers, baseUrl);
                        if(!canaryResult.ok) {
                            Log.d(TAG, "ntk_native_ack_canary_failed path=" + challengePath);
                        ...
                        Log.d(TAG, "ntk_native_ack_canary_timeout path=" + challengePath);
                    }
                }
            }
    ...
    private void submitNtkAckChallengeLane(ExecutorCompletionService<NtkQuicFetcher.Result> completion,
                                           ...
                                           int attempt,
                                           int lane) {
        futures.add(completion.submit(() -> NtkQuicFetcher.fetchWithEngine(
                engine,
                executor,
                baseUrl + "/api/ad/challenge",
                agent,
                getCookieHeaderForNtkPath(path),
                headers,
                "POST",
                challengeBytes,
                NTK_ACK_CHALLENGE_TIMEOUT_MS)));
        ...
        try {
            for(int attempt = 0; attempt < NTK_ACK_REQUEST_ATTEMPTS; attempt++) {
                ...
                canary = NtkQuicFetcher.fetchWithEngine(engine, executor, baseUrl + "/api/ad/canary", agent,
                        getCookieHeaderForNtkPath(challengePath), headers, "POST", canaryBytes, NTK_ACK_CONFIRM_TIMEOUT_MS);
    ...
    private int fetchNtkAckImpressions(HttpEngine engine, ExecutorService executor, String baseUrl,
        ...
        for(int i = 0; i < impressionUrls.length(); i++) {
            ...
            if(rawUrl.length() == 0)
                continue;
            final int index = i;
            final String impressionUrl = rawUrl.startsWith("http") ? rawUrl : baseUrl + rawUrl;
            futures.add(completion.submit(() -> {
                try {
                    NtkQuicFetcher.Result imp = NtkQuicFetcher.fetchWithEngine(engine, executor, impressionUrl, agent,
                            getCookieHeaderForNtkPath(challengePath), Collections.emptyMap(), "GET", null, NTK_ACK_CONFIRM_TIMEOUT_MS);
        ...
        int targetSeen = Math.max(1, Math.min(minSeen <= 0 ? 2 : minSeen, submitted));
        int seen = 0;
        for(int done = 0; done < submitted; done++) {
            try {
                Future<NtkImpressionResult> future = completion.poll(done == 0 ? 1400L : 250L, TimeUnit.MILLISECONDS);
                if(future == null)
                    break;
