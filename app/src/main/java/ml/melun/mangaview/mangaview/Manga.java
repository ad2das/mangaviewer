...
public class Manga {
        ...
        try {
            ...
            if(allowGeneratedImages) {
                ...
                if(generatedPrimaryValidationMiss[0]) {
                    ...
                    if(isUsableNtkApiPage(completedPage)
                            && addNtkApiViewerImageCandidates(client, completedPage.body, path, seenImages, false)) {
            ...
            if(blockedPage) {
                ...
                addNtkViewerShellGeneratedImageCandidates(client, page.body, path, seenImages);
    ...
    private boolean addNtkSlugViewerMetaImageCandidates(CustomHttpClient client, String normalized,
        ...
        Matcher pathMatcher = Pattern.compile("^/webtoon/\\d{6,}/([^/?#]+)").matcher(path == null ? "" : path);
        if(!pathMatcher.find())
            return false;
