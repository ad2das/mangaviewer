...
class ReaderSession(
    ...
    private fun shouldDeferGeneratedEarlyExpansionBeforeFirstBitmap(target: Manga, urls: List<String>): Boolean {
        if (!isNtkWebtoonSource(target, title)) return false
    ...
    private fun ntkGeneratedEarlyExpandCount(target: Manga, currentCount: Int, knownCount: Int): Int {
        if (target.baseMode == MTitle.base_comic) {
            val comicLimit = maxOf(currentCount, NTK_COMIC_GENERATED_EARLY_DISPLAY_PAGES)
            return when {
        ...
        return when {
            ...
            else -> NTK_UNKNOWN_GENERATED_DISPLAY_THRESHOLD
        }
    }
    ...
    private fun requestInitialContinuousPagesFromEarlyUrls(startPage: Int, urlCount: Int) {
        ...
        val last = minOf(count - 1, urlCount - 1, startPage + NTK_INITIAL_CONTINUOUS_REQUIRED_PAGES - 1)
    ...
    private fun appendInitialNtkUrlsAfterEarlyInstall(target: Manga, urls: List<String>, loadStartedAt: Long) {
        if (cancelled.get() || urls.isEmpty()) return
        var fullRefs = pageRefsForImages(target, urls)
        ...
        logNtkRepositoryStage(
            ...
            "from=$startIndex,total=$total,ms=${SystemClock.elapsedRealtime() - loadStartedAt}"
        )
        val posted = main.post {
            try {
                if (!cancelled.get()) listener.onPagesAppended(total)
            } finally {
    ...
    private fun replaceNtkBoardUploadsWithGeneratedSeed(
        ...
        Log.d(
            ...
            "ntk_generated_full_replace_board_uploads path=${target.ntkEpisodePath}," +
                "count=${fullRefs.count { isNtkBoardUploadImageUrl(it.image) }},seed=${seed.substringAfter("://")}"
        )
        return replaced
    }
    ...
    private fun shouldRefreshInitialNtkInstalledRefs(fullRefs: List<PageRef>): Boolean {
        ...
        if (pages.size != fullRefs.size) return false
    ...
    private fun startInitialForegroundStreamIfNeeded(
        ...
        val started = try {
            ReaderImageCache.startForegroundStreamFetch(appContext, target, image, imageCancellation, anchorHedge = false)
        } catch (e: Exception) {
            recordIfUnexpected(e)
            false
        }
    ...
    private fun requestWindow(first: Int, last: Int, anchor: Int, busy: Boolean, retainWindow: Boolean) {
        ...
        if (ntkWebtoon) {
            val visibleFirst = max(safeFirst, windowAnchor - NTK_WEBTOON_IDLE_VISIBLE_DECODE_RADIUS)
            val visibleLast = minOf(safeLast, windowAnchor + NTK_WEBTOON_IDLE_VISIBLE_DECODE_RADIUS)
            val decodeFirst = visibleFirst
            val decodeLast = minOf(safeLast, windowAnchor + NTK_WEBTOON_IDLE_DECODE_AHEAD)
    ...
    private fun requestPage(index: Int, busy: Boolean, anchor: Boolean, generation: Int = windowGeneration.get()) {
        ...
        if (activeWidth >= effectiveTargetWidth && loading.contains(index)) return
                ...
                try {
                    if (!shouldSkipStalePage(index, generation, false)) {
                        prefetchImageFile(
                            index,
                            page,
                            foreground = generation == FOREGROUND_PRIME_WARM_GENERATION && isNtkGeneratedPageRef(index)
                        )
    ...
    private fun runNtkGeneratedFullEpisodeBytePrefetch() {
        ...
        val defaultFirst = minOf(refs.lastIndex, anchor + ntkGeneratedInitialLimitedWarmPages() + 1)
    ...
    private fun ntkGeneratedImageUrlForTarget(seed: String, target: Manga, page: Int): String? {
        ...
        val episodeId = imageEpisodeId.takeIf { it.matches(Regex("\\d+")) } ?: pathEpisodeId
        ...
        val targetPrefix = when {
            seedPrefix.contains("/blacktoon/episodes/", ignoreCase = true) -> {
                ...
                "https://i.toonflix.app/blacktoon/episodes/$seedWorkId/$episodeId/"
            }
            seedPrefix.contains("/wt/episodes/", ignoreCase = true) -> {
                val seedWorkId = Regex("/wt/episodes/([^/]+)/").find(seedPrefix)
                    ...
                    ?.getOrNull(1)
                    ?: return ntkGeneratedSiblingImageUrl(seed, page)
                "https://i.toonflix.app/wt/episodes/$seedWorkId/$episodeId/"
            }
            else -> {
                ...
                "https://i.toonflix.app/$segment/$seedWorkId/$episodeId/"
            }
        }
        return "${targetPrefix}p${page.toString().padStart(3, '0')}.${seedMatch.groupValues[2]}${seedMatch.groupValues[3]}"
    }
    ...
    private fun shouldUseForegroundFetch(
        ...
        if (generation == FOREGROUND_PRIME_WARM_GENERATION && ntk) {
            return index <= current + foregroundAhead
        }
    ...
    private fun retryTransientNtkGeneratedPageError(index: Int, page: PageRef, e: Exception): Boolean {
        ...
        val delayMs = NTK_GENERATED_TRANSIENT_RETRY_DELAY_MS * attempt
        Log.d(
            TAG,
            "retry_transient_generated_page_error index=$index,source=${page.sourceIndex},attempt=$attempt," +
                "delayMs=$delayMs,path=${page.manga.ntkEpisodePath},image=$image,error=${e.javaClass.simpleName}:${e.message}"
        )
        main.postDelayed({
            if (!cancelled.get() && pageRef(index) == page) {
                requestPage(index, busy = true, anchor = false, generation = PRIME_WARM_GENERATION)
    ...
    private companion object {
        ...
        private const val NTK_COMIC_GENERATED_EARLY_DISPLAY_PAGES = 4
        private const val NTK_INITIAL_PRIORITY_START_OFFSET = 1
        private const val NTK_INITIAL_CONTINUOUS_REQUIRED_PAGES = 10
        ...
        private const val NTK_WEBTOON_INITIAL_BYTE_PREFETCH_AHEAD_PAGES = 8
        private const val NTK_GENERATED_INITIAL_LIMITED_WARM_PAGES = 64
        private const val NTK_GENERATED_INITIAL_LIMITED_FOREGROUND_PAGES = 24
        private const val NTK_GENERATED_INITIAL_LIMITED_BUSY_PAGES = 24
        private const val NTK_WEBTOON_GENERATED_INITIAL_LIMITED_WARM_PAGES = 4
        private const val NTK_WEBTOON_GENERATED_INITIAL_LIMITED_FOREGROUND_PAGES = 4
        private const val NTK_WEBTOON_GENERATED_INITIAL_LIMITED_BUSY_PAGES = 4
        ...
        private const val NTK_WEBTOON_BUSY_WINDOW_AFTER = 16
        private const val NTK_WEBTOON_BUSY_DIRECTIONAL_DECODE_AHEAD = 4
        private const val NTK_WEBTOON_BUSY_VISIBLE_DECODE_RADIUS = 2
        private const val NTK_WEBTOON_IDLE_VISIBLE_DECODE_RADIUS = 2
        private const val NTK_WEBTOON_IDLE_DECODE_AHEAD = 4
        ...
        private const val NTK_GENERATED_FULL_BYTE_PREFETCH_BATCH_DELAY_MS = 220L
        ...
        private const val NTK_GENERATED_BUSY_SOURCE_PREFETCH_AFTER = 4
