...
class ReaderSession(
    ...
    fun start() {
        network.execute {
            try {
                ...
                images.clear()
                images.addAll(urls)
                main.post { if (!cancelled.get()) listener.onPagesReady(urls.size) }
                requestWindow(0, minOf(urls.lastIndex, ReaderPipelinePolicy.INITIAL_WINDOW_AFTER), 0, false)
            } catch (e: Exception) {
            ...
            try {
                ...
                decode.execute {
                    ...
                    try {
                        ...
                        val bitmap = decodeFile(index, file, targetWidth)
    ...
    private fun decodeFile(index: Int, file: File, targetWidth: Int): Bitmap {
