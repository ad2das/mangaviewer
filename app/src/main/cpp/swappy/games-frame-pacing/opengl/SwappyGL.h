...
class SwappyGL {
   private:
    ...
    struct TestConstructorTag {};

    friend class SwappyGLFixedAdmissionTestPeer;
   ...
   public:
    ...
    static bool reserveFixedFrameForNtk(
        std::uint64_t workGeneration,
        SwappyFixedReservationReceipt* outReceipt);
    static bool abortFixedReservationForNtk(std::uint64_t workGeneration);
    static bool markReservedExternalGpuReadyForNtk(
        std::uint64_t workGeneration,
        const SwappyFixedExternalTransportReady* transportReady);
    static SwappyFixedCommitStatus claimPreparedExternalFixedFrameForNtk(
        const SwappyFixedOpportunityIdentity* expectedOpportunity,
        const SwappyFixedExternalTransportReady* transportReady,
        SwappyFixedExternalClaim* outClaim);
    static bool recordExternalLatchObservationForNtk(
        const SwappyFixedLatchObservationV1* observation);
    static bool commitExternalFixedSubmissionForNtk(
    ...
    static bool abortExternalFixedClaimForNtk(std::uint64_t claimToken);
    static bool hasExternalFixedClaimForNtk();
    static bool markFixedPhaseSubmissionFailureForNtk();
    ...
    static void setFixedNonPipelineMode(std::chrono::nanoseconds swapDuration);

    static int32_t getPipelineModeForNtk();
    static bool isFixedNonPipelineModeForNtk();
    static bool isBlockingWaitEnabledForNtk();
    static bool hasFatalPacingErrorForNtk();
    static bool isFixedPhaseConfigurationValidForNtk();
    static bool getFixedPhaseTelemetryForNtk(
   ...
   private:
    SwappyGL(const SwappyCommonSettings &settings, TestConstructorTag);
    ...
    SwappyFixedCommitStatus claimPreparedExternalFixedFrameInternal(
        const SwappyFixedOpportunityIdentity& expectedOpportunity,
        const SwappyFixedExternalTransportReady& transportReady,
        SwappyFixedExternalClaim* outClaim);
    bool commitExternalFixedSubmissionInternal(
        const SwappyFixedExternalClaim& claim,
        const SwappyFixedExternalSubmission& submission,
        SwappyFixedExternalSubmissionReceipt* outReceipt);
    ...
    std::mutex mExternalSubmissionMutex;
    std::optional<std::uint64_t> mExternalPreparedWorkGeneration;
    std::optional<ExternalClaimState> mExternalClaim;
