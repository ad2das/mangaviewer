...
`Build APK Artifact` and `Release APK` default to a read-only Gradle cache so builds skip the slower cache-save post step. Turn on `refresh_gradle_cache` only when dependencies or Gradle inputs changed and the cache needs to be warmed again.
