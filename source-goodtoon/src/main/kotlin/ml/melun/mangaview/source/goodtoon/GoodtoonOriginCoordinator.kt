...
/** Owns GoodToon origin discovery and publication for one source instance. */
internal class GoodtoonOriginCoordinator(
    ...
    suspend fun resolve(path: String): String {
        require(path.startsWith('/')) { "GoodToon path must be absolute" }
        return current() + path
    }
    ...
    private companion object {
        fun normalizeOrigin(value: String): String {
            ...
            require(!uri.host.isNullOrBlank()) { "GoodToon origin must include a host" }
